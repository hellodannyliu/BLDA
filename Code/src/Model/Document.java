...
//			ComUtil.uniqe(items);
	...
	//public HashMap<String, Integer> wordMap;
		...
		// initial data from tagged docs
		public Document(String Dir, String id, HashMap<String, Integer> wordMap, 
				HashMap<String, Integer> itemMap, Model model, int userNo) {
			this.id = id;
			String line;
			ArrayList<String> datalines = new ArrayList<String>();
			FileUtil.readLines(Dir, datalines);
			...
			// z and y
			model.Z[userNo] = new int[datalines.size()];
			model.y[userNo] = new boolean[datalines.size()][];
	...
	//			System.out.println(line);
				...
				// get topic
				int topic = Integer.parseInt(tokens.get(1).replace(":", "").trim());
				model.Z[userNo][lineNo] = topic;
				...
				ArrayList<Integer> items = new ArrayList<Integer>();
				if(tokens.size() > 2) {
					if(itemMap.containsKey(tokens.get(tokens.size() - 1)))
						items.add(itemMap.get(tokens.get(tokens.size() - 1)));
					else
						System.err.println("Doesn't contains item: " + tokens.get(tokens.size() - 1));
					for(int tokenno = 2; tokenno < tokens.size() - 1; tokenno++) {
						...
						if(wordMap.containsKey(wordToken)) {
							...
							// add y value
							int tmp = Integer.parseInt(word.substring(index+1));
							if(tmp == 0)
								ylabel.add(false);
							else
								ylabel.add(true);
				...
				model.y[userNo][lineNo] = new boolean[words.size()];
				docWords[lineNo] = new int[words.size()];
				for(int w = 0; w < words.size(); w++) {
					docWords[lineNo][w] = words.get(w);
					model.y[userNo][lineNo][w] = ylabel.get(w);
				}
				docItems[lineNo] = new int[items.size()];
				for(int w = 0; w < items.size(); w++)
					docItems[lineNo][w] = items.get(w);
	...
	public Document(String Dir, String id,
			HashMap<String, Integer> wordMap, int[][] nTW, float[][] vPhi, int [][]z, boolean[][][] y, 
			int userNo) {
		this.id = id;
		String line;
		ArrayList<String> datalines = new ArrayList<String>();
